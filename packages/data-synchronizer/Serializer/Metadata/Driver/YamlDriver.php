...
/**
 * Modified YamlDriver from JMS Serializer.
 * Extracts metadata for inherited properties instead of using parent class mapping.
 *
 * TODO: Try to fix deserialization rather than generating custom metadata with this driver.
 */
class YamlDriver extends AbstractFileDriver
{
    ...
    public function __construct(
        ...
        private ParserInterface $typeParser = new Parser(),
    ...
    protected function loadMetadataFromFile(\ReflectionClass $class, string $file): ?BaseClassMetadata
    {
        ...
        $exclusionPolicy = strtoupper($config['exclusion_policy'] ?? '') ?: 'NONE';
        $excludeAll = isset($config['exclude']) && $config['exclude'];
        $classAccessType = $config['access_type'] ?? PropertyMetadata::ACCESS_TYPE_PROPERTY;
        $readOnlyClass = isset($config['read_only']) && $config['read_only'];
        ...
        if (\array_key_exists('virtual_properties', $config)) {
            foreach ($config['virtual_properties'] as $methodName => $propertySettings) {
                ...
                $pName = empty($propertySettings['name']) ? $virtualPropertyMetadata->name : $propertySettings['name'];
     ...
     */
    private function getCallbackMetadata(\ReflectionClass $class, array|string $config): array
    {
        if (\is_string($config)) {
            $config = [$config];
        } elseif (!\is_array($config)) {
            throw new InvalidMetadataException(\sprintf('callback methods expects a string, or an array of strings that represent method names, but got %s.', json_encode($config['pre_serialize'], \JSON_THROW_ON_ERROR)));
        }
